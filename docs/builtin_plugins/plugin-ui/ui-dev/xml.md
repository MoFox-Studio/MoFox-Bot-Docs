...
完整组件属性表见 [XML 组件参考](./components)；HTML 轨（命令式）见 [HTML 开发](./html)。
...
`sys-text` 通过**默认插槽**显示文本，不要写 `<sys-text value="...">`，要写 `<sys-text>...</sys-text>`。详见 [组件参考](./components#sys-text)。
:::
...
XML 轨内置 25 个 Material Design 3 风格组件。下面是按分类的速览，**完整属性、事件、用法见 [XML 组件参考](./components)**。
...
::: tip 通用属性
所有组件都支持 `hidden`、`disabled`、`mobile-only`、`desktop-only`、`bind:prop`、`on-*` 等通用属性，详见 [组件参考 · 通用属性](./components#通用属性)。
:::
...
- [XML 组件参考](./components) — 所有内置 XML 组件的完整属性、事件、用法
- [HTML 开发](./html) — 用原生 HTML/CSS/JS 写自定义页面（命令式）
- [sys API](./sys-api) — `sys` 桥接对象完整 API（HTML 轨核心，XML 轨也可用）
