...
| 模块 | 说明 |
| --- | --- |
| [Action API](./action-api) | Action 组件查询、Schema 获取与缓存 |
| [Adapter API](./adapter-api) | 适配器查询与状态检查 |
| [Agent API](./agent-api) | Agent 组件查询与可用工具管理 |
| [Chat API](./chat-api) | Chatter 组件查询、实例管理与流绑定 |
| [Command API](./command-api) | 命令匹配、注册与帮助信息 |
| [Config API](./config-api) | 插件配置的加载与重载 |
| [Database API](./database-api) | CRUD、查询构建、聚合统计、分页、批量迭代 |
| [Event API](./event-api) | 事件发布、处理器注册与临时监听器 |
| [LLM API](./llm-api) | LLM 请求创建、模型集获取与工具注册 |
| [Log API](./log-api) | 日志记录器创建 |
| [Media API](./media-api) | 图片/表情识别与信息管理 |
| [Message API](./message-api) | 消息查询、计数与可读格式化 |
| [Permission API](./permission-api) | 用户身份标识生成 |
| [Plugin API](./plugin-api) | 插件加载与查询 |
| [Prompt API](./prompt-api) | 提示词模板注册与系统提醒 |
| [Router API](./router-api) | HTTP Router 查询与挂载信息 |
| [Send API](./send-api) | 文本、图片、Markdown 发送与广播 |
| [Service API](./service-api) | Service 组件查询 |
| [Storage API](./storage-api) | JSON 存储与 PluginDatabase |
| [Stream API](./stream-api) | 聊天流缓存管理与上下文操作 |
