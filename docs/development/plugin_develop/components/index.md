...
| 类型 | 说明 |
| --- | --- |
| `Wait(time=None)` | 等待新消息（无超时），由新消息或子代理恢复 |
| `Wait(time=30)` | 等待 30 秒后由框架主动恢复生成器 |
| `Success("完成")` | 对话成功结束，可携带 `data`、`step_data` |
| `Failure("出错")` | 对话失败结束，可携带 `exception`、`step_data` |
| `Stop(time=60)` | 结束并冷却 60 秒；支持 `direct_message_wake_enabled` / `direct_message_wake_probability` |

所有结果均支持可选的 `step_data: dict[str, Any] | None` 字段，用于在步进完成后由框架发布 `ON_CHATTER_STEP` / `AFTER_CHATTER_STEP` 通知事件。
...
- **基类**：`BaseAgent`
- **作用**：是 Chatter 的任务协助者,拥有专属的私有 usables 套件,可以编排多个工具完成复杂任务。
- **使用场景**：编排多个工具完成复杂任务。

详见 [Agent components](./agent)。
...
| `ON_MESSAGE_SENT` / `AFTER_MESSAGE_SENT` | 消息发送完成时 |
| `ON_NOTICE_RECEIVED` | 收到通知事件时 |
| `ON_ALL_PLUGIN_LOADED` | 所有插件加载完成时 |
| `ON_PLUGIN_UNLOADED` | 插件卸载时 |
| `ON_CHATTER_STEP` / `AFTER_CHATTER_STEP` | Chatter 单步执行前后 |
| `BEFORE_LLM_REQUEST` / `AFTER_LLM_REQUEST` / `ON_LLM_REQUEST_FAILED` | LLM 请求生命周期 |
| `BEFORE_TOOL_CALL` / `AFTER_TOOL_CALL` / `ON_TOOL_CALL_FAILED` | 工具调用生命周期 |
| `BEFORE_ACTION_CALL` / `AFTER_ACTION_CALL` / `ON_ACTION_CALL_FAILED` | 动作调用生命周期 |
| `BEFORE_COMMAND_EXECUTE` / `AFTER_COMMAND_EXECUTE` / `ON_COMMAND_EXECUTE_FAILED` | 命令执行生命周期 |

完整事件列表详见 [EventHandler components](./event-handler)。
