...
- 版本兼容性声明是**按需填写**的（两个字段是「或」关系，用到哪个就写哪个，都不需要也可不写——此时仅警告但加载）：
  - `api_version`：声明插件用到的**插件 API 模块**版本，按 20 个 `*_api` 模块逐一校验。**你的插件只要 `import` 了任何 `*_api` 模块，就应声明此项**。支持字符串（最简，等价于对全部模块应用同一要求）与 dict（精确声明用到的模块）两种形式。
  - `min_core_version`：声明插件依赖的**核心能力**版本（如某些新事件、新内核接口），基于 `CORE_VERSION` 做 `>=` 比较。**只有当插件用到「必须更高版本核心才支持」的能力时才需要写**；用稳定核心功能则可省略。

  两个字段各司其职、同等判断：声明了哪一项就校验哪一项，只要任一项声明且不满足即拒绝注册。本例同时列出两项是为了让你认识这两个字段；实际开发中按上面的规则按需填写即可。详见 [manifest.json 格式说明](../../manifest.md)。
...
- `api_version` 与 `min_core_version` 是否显式填写。
- JSON 格式本身是否正确。
