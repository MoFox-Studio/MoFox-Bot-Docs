...
-   **`1. 启动服务组合`**: **（推荐）** 提供预设的服务组合。选择 `内置适配器组合` 可以一键启动 `MoFox_Bot主程序` 和 `Napcat 服务`，这是让 Bot 在 QQ 平台运行起来的标准方式。
-   **`2. 启动 MoFox_Bot 主程序`**: 单独启动 Bot 的核心逻辑服务。
...
-   **Q: 我不小心把主控面板关了，但 Bot 还在运行，如何关闭它？**
    A: Bot 的每个服务（如 `MoFox_Bot 主程序`、`Napcat 服务`）都运行在独立的命令行窗口（通常是 PowerShell 或 CMD 窗口）中。要关闭某个服务，**直接关闭对应的那个命令行窗口**即可。
