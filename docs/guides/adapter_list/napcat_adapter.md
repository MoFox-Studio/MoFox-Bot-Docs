...
本文档将指导你如何独立配置和使用 Napcat Adapter，使其能够连接到你的 NapCat 客户端，并将消息转发到 Neo-MoFox。

## 什么是独立适配器？

独立适配器意味着 Napcat Adapter 作为一个独立的进程运行，它通过 WebSocket 与 NapCat 和 Neo-MoFox 进行通信。这种部署方式提供了更好的灵活性和稳定性。
