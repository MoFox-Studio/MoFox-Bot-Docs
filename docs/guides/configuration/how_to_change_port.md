...
1.  **OneBot 适配器端口** (默认为 `8095`)：这是 Neo-MoFox 与 Napcat QQ 客户端建立连接的端口。
2. **HTTP API 端口** (默认为 `8000`)：这是 Neo-MoFox 的 插件 提供 HTTP API 服务的端口。


## 场景1：修改 OneBot 适配器端口 (例如 `8095` 端口被占用)
...
1.  打开 `config/plugins/onebot_adapter/config.toml` 文件。
2.  找到 `[onebot_server]` 配置节。
...
> **💡 总结**:
> *   修改 **OneBot 连接端口**，需要同时改 `Napcat QQ 客户端` 和 `onebot_adapter` 的配置。
