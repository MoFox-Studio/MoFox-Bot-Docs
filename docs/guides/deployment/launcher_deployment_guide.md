...
| **WebUI 自动打开** | 启动 SnowLuma 时自动在浏览器中打开 WebUI | 启用 |
...
配置 SnowLuma 与 Neo-MoFox 的通信参数和安全认证。

##### WebSocket 配置

| 字段 | 说明 | 推荐值 |
|------|------|--------|
| **WebSocket 端口** | SnowLuma 与 Neo-MoFox 通信的端口 | `8095`（默认） |
...
| 组件 | 说明 | 推荐 |
|------|------|------|
| **安装 SnowLuma** | QQ 协议端，负责与 QQ 服务器通信 | ✅ 推荐 |
| **安装 WebUI** | SnowLuma 的图形化管理面板，可查看账号状态、扫码登录等 | ✅ 推荐 |

> [!NOTE]
> **关于组件选择**：
> - 如果不安装 SnowLuma，仅部署 Neo-MoFox 核心，无法连接 QQ
> - WebUI 提供可视化的账号管理界面，方便扫码登录和查看连接状态
> - **Linux 用户注意**：Launcher 目前对 Linux 平台的 SnowLuma 支持有限，该选项会被自动禁用
...
5. **下载 SnowLuma**（如果选择）- 下载 SnowLuma 协议端
...
- 包名称、版本、作者、描述
- 预装的插件列表及版本
- 包含的组件（Neo-MoFox 主程序、SnowLuma、配置文件、数据文件等）
- 创建时间和整合包版本
...
| **WebSocket 端口** | SnowLuma 与 Neo-MoFox 通信端口 | `8095`（默认） |
...
| **安装 SnowLuma** | QQ 协议端，负责与 QQ 服务器通信 | ✅ Windows 推荐 |
...
>   - 整合包可能已包含 SnowLuma，则会跳过下载步骤
>   - **Linux 用户注意**：Launcher 目前不支持在 Linux 系统上自动安装和管理 SnowLuma，该选项在 Linux 下会被自动禁用。Linux 用户需要手动部署 SnowLuma
...
**整合包信息**：
- 包名称、版本、作者
- 包含内容（Neo-MoFox、SnowLuma、插件、配置、数据等）
...
1. 验证整合包
2. 解压整合包
3. 复制文件（Neo-MoFox、SnowLuma、插件、配置等）
4. 克隆代码仓库（如果整合包未包含主程序）
5. 创建虚拟环境
6. 安装 Python 依赖
7. 生成配置文件（替换占位符）
8. 下载 SnowLuma（如果选择安装且整合包未包含）
9. 克隆 WebUI（如果选择安装）
10. 注册实例
...
1. 找到新创建的实例卡片
2. 点击"启动"按钮（▶️ 图标）
3. Launcher 会启动 Neo-MoFox 和 SnowLuma（如果安装）
4. 状态指示器变为黄色（启动中），然后变为绿色（运行中）

**首次启动**：
- SnowLuma 会打开 WebUI 页面（浏览器）
