...
- 芯片架构: ARM64（目前主流手机均满足，即 AArch64）
- 存储空间: 至少 8GB 可用空间
- 运行内存：最低 6GB 推荐12GB
...
在安装好 ZeroTermux 之后，请**务必先完成以下保活设置**，再进行后续的 proot 和 Ubuntu 安装。由于安卓系统会在后台自动关闭应用以节省电量和内存，如果不做保活设置，你的 ZeroTermux 随时可能被系统"杀死"，导致部署中断或机器人离线，这里以欧加真系的ColorOS举例。
...
1. **找到自启动设置**:
    ...
    ![设置搜索自启动](/android保活/设置搜索自启动.jpeg)

2. **开启自启动**:
    - 点击 **"自启动"** 进行添加。

    ![点击自启动选项](/android保活/点击自启动选项.jpeg)

    - 在自启动选项中，找到 **ZeroTermux**，将其开关打开。

    ![勾选 ZeroTermux](/android保活/勾选zerotermux.jpeg)

#### 2.2 关闭耗电管理限制

1. **进入应用详情**:
    - 长按 **ZeroTermux** 的应用图标，在弹出的菜单中点击 **"应用详情"**。

    ![长按点击应用详情](/android保活/长按点击应用详情.jpeg)

2. **修改耗电管理**:
    - 在应用详情页面，点击 **"耗电管理"** 选项。

    ![应用详情点击耗电管理](/android保活/应用详情点击耗电管理.jpeg)

    - 将耗电管理设置为 **"不限制"**，防止系统因省电策略而终止 ZeroTermux 的后台运行。

    ![耗电管理选择不限制](/android保活/耗电管理选择不限制.jpeg)

#### 2.3 锁定后台

- 打开手机的 **多任务视图**（通常是按下底部的方块键或从屏幕底部上滑并停顿）。
- 找到 **ZeroTermux** 的卡片，向下滑动卡片将其 **加锁**。加锁后卡片上会出现一个小锁图标，这样系统就不会自动清理它了。

![下滑后台加锁](/android保活/下滑后台加锁.jpg)

:::warning 重要提醒
完成以上三步保活设置后，你的 ZeroTermux 才能在后台稳定运行。如果你发现机器人经常掉线，大概率是漏做了其中某一步，请重新检查。
:::
...
1. **首次启动**:
    - 确保你的命令行终端当前目录在 Neo-MoFox 项目文件夹中。
...
# uv run 自动使用虚拟环境，无需手动激活
# 4. 启动 Neo-MoFox (在 screen 会话中)
screen -dmS neomofox bash -c "uv run main.py; exec bash"
```
