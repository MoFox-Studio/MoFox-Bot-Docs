# <iconify-icon icon="material-symbols:window-sharp" height="36"></iconify-icon> Neo-MoFox Windows 部署指南 (内置适配器版)

## 概述

欢迎使用 Neo-MoFox，一个高度可定制化的 AI Bot 框架。

本指南将引导您在 Windows 环境下，使用项目**内置的 Napcat 适配器插件**完成 Neo-MoFox 的全部署流程。该方式是官方推荐的最佳实践，具有以下优势：
...
1.  **进入项目目录**:
    *   首先，确保你的命令行终端还开着，并且路径在 `Neo-MoFox_Deployment` 文件夹下。
...
> 2.  **权限不足**：尝试关闭当前的命令行窗口，然后**以管理员身份**重新打开一个新的终端，再进入项目目录、激活环境、执行安装命令。
