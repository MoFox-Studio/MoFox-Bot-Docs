...
| 项目 | 要求 | 说明 |
|------|------|------|
| **操作系统** | Windows 10/11 (64-bit) | 目前仅支持 Windows |
...
3. 运行安装程序时，**务必勾选底部的 "Add Python to PATH"**
4. 点击 "Install Now" 完成安装
5. 打开 **命令提示符** 或 **PowerShell**，输入以下命令验证：
   ```powershell
   python --version
   ```
   应显示类似 `Python 3.11.x` 或更高版本
...
| 文件名 | 说明 | 适合人群 |
|--------|------|----------|
| `Neo-MoFox Launcher-x.x.x-x64-setup.exe` | **安装版**（推荐） | 想要添加到开始菜单、桌面快捷方式 |
| `Neo-MoFox Launcher-x.x.x-x64-portable.exe` | 便携版 | 不想安装、直接运行，或使用 U 盘 |

> [!NOTE]
> 文件名中的 `x.x.x` 代表版本号（如 `1.0.0`），`x64` 代表 64 位系统
