...
## ⚠️ 首要步骤：配置你的主人 (Owner User)！

在开始使用权限系统之前，**最重要、最优先**的一步是在你的机器人配置文件中设置 **Owner 用户**。

### 什么是 Owner 用户？

Owner 用户是机器人的**最高管理员**，拥有所有权限，可以执行任何操作，不受权限系统的任何限制。你可以把它理解为机器人的“主人”。

### 为什么必须先配置 Owner 用户？

*   **初始授权**：权限管理指令（例如 `/system permission grant`）本身是需要权限才能使用的。如果没有一个初始的最高权限用户，就没人能够分配权限，整个权限系统就无法启动。
*   **安全保障**Owner 用户是管理机器人的最终保障，确保你永远不会因为错误的权限配置而失去对机器人的控制。

在你完成 Owner 用户的配置之前，请**不要**尝试使用任何权限管理指令，否则你只会收到“权限不足”的提示。

> **提示**: Owner 用户的配置在 `config/core.toml` 文件中完成。你需要找到 `[permissions]` 部分，然后将自己的用户 ID 添加到 `owner_list` 字段中。
>
> 下面是一个具体的配置示例：
>
> ```toml
...
># 值类型：list, 默认值：[]
>owner_list = ['qq:123456789']  # <-- 将 '123456789' 替换为你的实际用户ID
> ```
>
> **请务必注意格式**：
> *   使用**单层**方括号 `[platform:user_id]`。
...
在你配置好 Owner 用户之后，我们再来了解这个强大的权限系统。
...
作为 Owner 用户，或已被授予权限的管理员，你可以通过 `/system permission` 系列指令来管理权限。
...
A: 这意味着你没有执行该操作所需的“钥匙”（权限节点）。如果你是机器人的主人，请先检查是否已在配置文件中将自己设为 Owner 用户。如果不是，你需要联系机器人的管理员，让他使用 `/system permission grant` 指令授予你相应的权限。
...
```mermaid
graph TD
    A[开始：配置机器人] --> B{配置文件中是否已设置 Owner 用户?};
    B -->|否| C[打开配置文件 config/core.toml];
    C --> D[找到 owner_list 字段];
    D --> E[填入你自己的用户ID];
    E --> F[保存配置文件并重启机器人];
    B -->|是| G[机器人正常启动];
    F --> G;
