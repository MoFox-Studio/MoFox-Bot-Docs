...
1.  **发言后暂停 (Pause after Speaking)**：当 AI 主动发起一次“冒泡”或“发起聊天”后，它会**立刻暂停**该聊天的主动思考任务。这就像在说：“我已经开口啦，现在该等你回应了。”
...
*   `min_interest_score` & `max_interest_score`
    ...
    *   **类型**: `float`
    *   **建议**: 默认的 `0.0` 到 `1.0` 意味着所有情况都考虑。你可以适当提高 `min_interest_score` (比如 `0.2`) 来避免在 AI 完全不感兴趣时还强行思考。
...
*   `quiet_hours_start` & `quiet_hours_end`
    ...
    *   **类型**: `str`
    *   **建议**: 设置一个符合你作息的“深夜”时段，比如 `"00:00"` 到 `"07:00"`。

*   `active_hours_multiplier`
    *   **说明**: 在非安静的“活跃时段”，思考间隔的倍率。小于 1 表示更频繁，大于 1 表示更稀疏。
    *   **类型**: `float`
    *   **建议**: 如果你想让 AI 在白天更活跃，可以设置为 `0.7` 或 `0.8`。
...
*   `max_daily_proactive`
    ...
    *   **类型**: `int`
    *   **建议**: 如果你觉得 AI 有点过于热情，可以设置为 `5` 或 `10` 来限制一下。
